...
export const getAllUsers = async (req: Request, res: Response) => {
  try {
    ...
    const users = await User.find({ id: userId })
      .sort({ createdAt: -1 })
      .populate("branch", "branchName -_id")
...
export const disableUser = async (req: Request, res: Response) => {
  try {
    ...
    if (forms.modifiedCount > 0) {
      return res
        .status(200)
        .json({ ok: true, message: "User Status changed successfully" });
    } else {
      return res
        .status(422)
...
export const getSingleUserByID = catchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
    // Assuming validateMongooseId returns a boolean indicating whether the ID is valid
...
export const changePassword = catchAsyncErrors(
  async (req: ModifiedRequest, res: Response) => {
    ...
    user.resetPasswordExpire = undefined;
    await user.save();
    sendResponse("Password updated successfully", 200, res);
  }
);
...
export const deleteUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (form.deletedCount > 0) {
      return res
        .status(200)
        .send({ ok: true, message: `User deleted successfully.` });
    } else {
